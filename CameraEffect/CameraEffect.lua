--print("CameraEffect Loaded");
--[[
local MenuBarOrnaments = CreateFrame("Frame", "MenuBarOrnaments", UIParent)
local SizeX, SizeY = MainMenuBarArtFrame.LeftEndCap:GetSize()
local Tex = MainMenuBarArtFrame.LeftEndCap:GetAtlas();

local LeftEndTexture = MenuBarOrnaments:CreateTexture("LeftEnd", "ART")
LeftEndTexture:SetAtlas(Tex)
LeftEndTexture:SetSize(SizeX, SizeY);
LeftEndTexture:SetPoint("CENTER", MainMenuBarArtFrame.LeftEndCap, "CENTER", 0, 0);

local RightEndTexture = MenuBarOrnaments:CreateTexture("RighttEnd", "ART")
RightEndTexture:SetAtlas(Tex)
RightEndTexture:SetSize(SizeX, SizeY);
RightEndTexture:SetTexCoord(1, 0, 0, 1);
RightEndTexture:SetPoint("CENTER", MainMenuBarArtFrame.RightEndCap, "CENTER", 0, 0);

MenuBarOrnaments:SetFrameStrata("HIGH")
MenuBarOrnaments:Hide();

if MainMenuBarArtFrame.LeftEndCap:IsShown() then
    MenuBarOrnaments:Show();
...
local ScreenFilter = CreateFrame("Frame", "ScreenFilter", UIParent)
...
local SnowTexture = {};
SnowTexture = {
    [1] = "Interface/AddOns/AzerothAdventureAlbum/Features/CameraEffect/Snow-Ellipse",
    [2] = "Interface/AddOns/AzerothAdventureAlbum/Features/CameraEffect/Snow-Rectangle",
    [3] = "Interface/AddOns/AzerothAdventureAlbum/Features/CameraEffect/Snow-Triangle",
    [4] = "Interface/AddOns/AzerothAdventureAlbum/Features/CameraEffect/Snow-Round",
}
...
local SnowContainer = CreateFrame("Frame", "SnowContainer", UIParent)
...
function SnowEffect(switch)
    ...
    if AAASettings["Little Things"] and switch == true and IsOutdoors() and (not IsSubmerged()) then
        if SnowyLand[mapID] ~= nil and (Px > SnowyLand[mapID][2] and Px < SnowyLand[mapID][4]) and (Py > SnowyLand[mapID][3] and Py < SnowyLand[mapID][5]) then
            UIFrameFadeIn(ScreenFilter, 2, 0, 1);
    ...
    elseif switch == false then
        if ScreenFilter:IsShown() then
            UIFrameFadeOut(ScreenFilter, 1, ScreenFilter:GetAlpha(), 0);
        end
        animIn:Stop();
        animOut:Play();
    end
